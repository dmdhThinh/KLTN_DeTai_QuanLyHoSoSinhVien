...
// 🧩 Lịch học của giảng viên
export async function getLichHocGiangVien(req, res) {
  try {
    const { giangVienId, from } = req.query
    ...
    const sql = `
      ...
      ORDER BY lh.thu ASC, lh.ca ASC, lh.tiet_bat_dau ASC
    `
    const [rows] = await pool.execute(sql, [from, from, giangVienId, from, from, from, from])
    res.json(rows)
  } catch (err) {
    console.error('❌ Lỗi getLichHoc giảng viên:', err)
    res.status(500).json({ message: 'Lỗi server khi lấy lịch học giảng viên' })
  }
}

// 🧩 Lịch thi của giảng viên
export async function getLichThiGiangVien(req, res) {
  try {
    const { giangVienId, from } = req.query
    if (!giangVienId) return res.status(400).json({ message: 'Thiếu giangVienId' })
    if (!from) return res.status(400).json({ message: 'Thiếu ngày bắt đầu tuần (from)' })
    ...
    const sql = `
      ...
      ORDER BY lh.ngay_hoc ASC, lh.ca ASC
    `
    const [rows] = await pool.execute(sql, [giangVienId, from, from])
    res.json(rows)
  } catch (err) {
    console.error('❌ Lỗi getLichThi giảng viên:', err)
